...
class TexTransformExample(Scene):
    def construct(self):
        ...
        lines = VGroup(
            # 将多个参数传递给 Tex，这些参数看起来被连接在一起作为一个表达式,但整个 mobject 的每个 submobject 为其中的一个字符串
            # 例如，下面的 Tex 物件将有 5 个子物件，对应于表达式[A^2，+，B^2，=，C^2]
            Tex("A^2", "+", "B^2", "=", "C^2"),
            Tex("A^2", "=", "C^2", "-", "B^2"),# 这里同理
            # 或者，你可以传入关键字参数 isolate，其中包含一个字符串列表,这些字符串应该作为它们自己的子物件存在
        ...
        self.add(lines[0])
        # TransformMatchingTex 将源和目标中具有匹配 tex 字符串的部分对应变换
